/* COUNTER GAME : https://www.hackerrank.com/contests/iiits-ytp2026-3103/challenges/counter-game
...
6 is not a power of 2 so Louise reduces it by the largest power of 2 less than 6 - 4 = 2.
...
string counterGame(long n) {
    ...
    vector<long> vec;       // We are storing the powers of 2 in a vector, so that we can easily find the nearest power of 2 less than n. We can also do this by using bit manipulation, but here we are using vector to store the powers of 2 for better understanding.
    long temp = 1;
    while(temp <= n) {
        vec.push_back(temp);
        temp *= 2;
    ...
    while(n != 1) {
        ...
        // Every time we have to check because(i.e., do the below 2 steps) because take for example 63, then nearest is 32, so we pass on 31(63 - 32), which is again not a power of 2. So, we have to do the checking in every iteration.
        if(vec[N - 1] == n) 
            n = n / 2;
        else 
            n = n - vec[N - 1];
