...
Node* merge(Node* head, Node* sortedHead) {
	Node* mergeHead = new Node(-1);
	Node* mergeTail = mergeHead;

	while(head && sortedHead) {
		if(head -> data <= sortedHead -> data) {
			mergeTail -> child = head;
			head = head -> child;
		} else {   
			mergeTail -> child = sortedHead;
			sortedHead = sortedHead -> child;
		}
